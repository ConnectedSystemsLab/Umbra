	...
	while simulating:
		...
		for i, _satellite in enumerate(satellites):
			if _satellite.id in groundstation_satellite_mapping:
				distortion_ratio = 1 + noise_level * np.random.uniform(-1, 1)
				groundstation_id, bandwidth = groundstation_satellite_mapping[_satellite.id]
				_satellite.set_transmit(bandwidth * distortion_ratio)
...
def optimize(
		...
		sat_gs_assignment_record,
		args,
) -> tuple[list[dict[Any, tuple[Any, Any, Any]]], Any]:
	...
	for step in range(num_steps):
		...
		for _satellite in groundstation_satellite_mapping:
			flow_assignment[_satellite] = (
				...
				graph.edges[f'sat_{step}_{_satellite}', f'gs_{step}_{groundstation_satellite_mapping[_satellite][0]}'][
					'capacity'] / args.time_step
			)
	...
	for _step in range(num_steps):
		if _step < len(flow_record):
			...
			for _satellite in satellites:
				if _satellite.id in groundstation_satellite_mapping:
					_groundstation, bandwidth, capacity = groundstation_satellite_mapping[_satellite.id]
					distortion_ratio = 1 + noise_level * np.random.uniform(-1, 1)
					_satellite.set_transmit(min(capacity * distortion_ratio, bandwidth))
...
if __name__ == '__main__':
	...
	nominal_bandwidth = deepcopy(bandwidth_record)
	logger.info("Adding noise")
	for gs in bandwidth_record:
		for sat in bandwidth_record[gs]:
			for i in range(len(bandwidth_record[gs][sat])):
				bandwidth_record[gs][sat][i] *= random.uniform(1 - args.noise_level, 1 + args.noise_level)
	...
	for step in range(args.iterations):
		...
		_cloud_new, _gs_queue_record = execute(deepcopy(groundstation_collection), deepcopy(satellite_collection),
		                                       deepcopy(cloud),
		                                       _flow_result, args, noise_level=args.noise_level)
