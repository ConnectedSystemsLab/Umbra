	...
	while simulating:
		...
		for i, _satellite in enumerate(satellites):
			if _satellite.id in groundstation_satellite_mapping:
				...
				current_size, next_size = find_gs_by_id(groundstation_id, groundstations).get_buffer_size(), \
					find_gs_by_id(next_contact, groundstations).get_buffer_size()
				logger.debug(
					f'gs {groundstation_id} buffer size {current_size}, '
					f'next contact {next_contact} buffer size {next_size}')
				if current_size == 0:
					size_to_downlink = math.inf
				else:
					size_to_downlink = _satellite.get_cache_size() * next_size / (current_size + next_size)
				regularized_bandwidth = max(bandwidth, size_to_downlink / time_step)
				logger.debug(
					f'satellite {_satellite.id} regularized bandwidth '
					f'{regularized_bandwidth} ratio {regularized_bandwidth / bandwidth}')
				_satellite.set_transmit(regularized_bandwidth)
