...
def optimize(
	...
	for step in range(num_steps):
		if step >= crash_step:
			groundstations = [gs for gs in groundstations if gs.get_id() not in crash_gs]
...
def execute(groundstations: List[AbstractGroundstation], satellites: List[AbstractSatellite], _cloud, flow_record,
	...
	for _step in range(num_steps):
		if _step >= crash_step:
			groundstations = [_gs for _gs in groundstations if _gs.get_id() not in crash_gs]
...
if __name__ == '__main__':
	...
	parser.add_argument('--result_dir', type=str, default='data/network_crash_oracle/')
	parser.add_argument('--log_file', type=str, default='log/network_crash_oracle.log')
